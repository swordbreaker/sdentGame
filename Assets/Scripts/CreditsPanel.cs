...
public class CreditsPanel : MonoBehaviour
{
    ...
    [SerializeField] private float _delayBeforeSwitch = 2.0f;

    private void Start()
    {
        float scrollDuration = 4600f / Screen.height * _secondsPerScreen;
        float creditsDuration = scrollDuration + _delayBeforeSwitch;
        GetComponent<RectTransform>().DOLocalMoveY(4600f, 4600f / Screen.height * _secondsPerScreen).SetEase(Ease.Linear);
        StartCoroutine(FadeOutAudioListener(2, 0.1f, scrollDuration));
